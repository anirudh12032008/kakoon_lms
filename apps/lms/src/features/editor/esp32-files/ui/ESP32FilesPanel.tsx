  ...
  return createPortal(
    ...
    >
      ...
      <div
        ...
        style={{ background: "#13131a", borderBottom: "1px solid #1e1e28" }}
      ...
      <div className="flex items-center gap-0.5 px-3 py-2 bg-[#0e0e14] border-b border-[#1a1a20] flex-shrink-0 flex-wrap min-h-[36px]">
        {crumbs.map((c, i) => {
          ...
          return (
            <span key={c.path} className="flex items-center">
              {i > 0 && <ChevronRight className="w-3 h-3 text-zinc-700 mx-0.5 flex-shrink-0" />}
              <button
                onClick={() => { if (!isLast) { setPath(c.path); scan(c.path); } }}
                className={`text-[10px] font-mono px-1.5 py-0.5 rounded transition-all ${
                  isLast ? "text-emerald-400 font-bold cursor-default" : "text-zinc-500 hover:text-zinc-200 hover:bg-white/5"
                }`}
              >{c.label}</button>
            </span>
          );
        ...
        <button
          ...
          title="Refresh"
          className="ml-auto p-1 rounded-lg text-zinc-500 hover:text-emerald-400 hover:bg-emerald-500/10 disabled:opacity-40 transition-all"
        >
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5 min-h-0">
        {entries.length === 0 && !scanning ? (
          <div className="text-center py-10 flex flex-col items-center gap-3">
            <HardDrive className="w-8 h-8 text-zinc-800" />
            <p className="text-[11px] text-zinc-600 max-w-48 text-center leading-relaxed">
          ...
          entries.map(entry => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-[11px] font-mono truncate ${entry.isDir ? "text-amber-300" : "text-zinc-200"}`}>
                  {entry.name}
                </p>
                {!entry.isDir && <p className="text-[9px] text-zinc-600">{fmtSize(entry.size)}</p>}
              </div>
              {entry.isDir
                ...
                : (
                  <button
                    ...
                    className="p-1.5 rounded-lg text-zinc-700 hover:text-red-400 hover:bg-red-500/10 disabled:opacity-40 transition-all flex-shrink-0"
