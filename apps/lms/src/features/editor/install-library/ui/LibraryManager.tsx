      ...
      <div className="flex items-center gap-3 px-3 py-2.5 border-b border-[var(--k-base-200)] last:border-0 hover:bg-[var(--k-base-100)] transition-colors group">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            ...
            {lib.size && <span className="text-[9px] text-[var(--k-dim)]">{lib.size}</span>}
          </div>
          <p className="text-[10px] text-[var(--k-muted)] leading-tight truncate">{lib.description}</p>
          {busy && (
        ...
        <button
          ...
          className={`flex-shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
              ...
              : "bg-[var(--k-base-400)] border border-white/10 text-[var(--k-text)] hover:bg-[var(--k-base-400)]"
          }`}
        >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--k-border)] bg-gradient-to-r from-violet-500/10 to-fuchsia-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-[var(--k-muted)]">Install MicroPython libraries to your ESP32</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-[var(--k-muted)] hover:text-[var(--k-text)] hover:bg-[var(--k-base-400)] transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {requiredLibs.length === 0 && (
            <div className="mx-4 mt-4 px-4 py-3 rounded-xl border border-[var(--k-base-400)] bg-[var(--k-base-100)] text-center">
              <p className="text-xs text-[var(--k-muted)]">No external libraries detected in your current code. Add hardware blocks to see what's needed.</p>
            </div>
          )}
          ...
          <div className="px-4 pt-4 pb-1">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-[var(--k-base-400)]/50 text-[var(--k-muted)] border border-[var(--k-border)]/50">
            ...
            <div className="flex items-center gap-2 bg-[var(--k-base-100)] border border-[var(--k-base-400)] rounded-xl px-3 py-2 mb-3">
              <Search className="w-3.5 h-3.5 text-[var(--k-muted)] flex-shrink-0" />
              <input
                value={search} onChange={(e) => setSearch(e.target.value)}
                placeholder="Search libraries…"
                className="bg-transparent text-xs text-white outline-none flex-1 placeholder:text-[var(--k-dim)]"
              />
            </div>
                ...
                <button key={cat} onClick={() => setExpandedCat(cat)}
                  className={`px-2.5 py-1 rounded-full text-[10px] font-bold transition-all ${
                    expandedCat === cat
                      ...
                      : "bg-[var(--k-border)] text-[var(--k-muted)] border border-[var(--k-border)] hover:border-[var(--k-dim)]"
                  }`}>
