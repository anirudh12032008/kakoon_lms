/**
 * 5×7 column font shared by the Matrix scroll-text generator and the OLED
 * text stamp. Each glyph is 5 column bytes; bit n of a byte = row n.
 */
export const FONT5: Record<string, number[]> = {
  ...
  ",":[0,80,48,0,0],"?":[2,1,81,9,6],"-":[8,8,8,8,8],"+":[8,8,62,8,8],
  ":":[0,54,54,0,0],"'":[0,5,3,0,0],"<":[8,20,34,65,0],">":[0,65,34,20,8],
};

/** Text → column bytes (1px gap between glyphs). */
export function textToColumns(text: string): number[] {
  return text.toUpperCase().split("").flatMap((c) => [...(FONT5[c] ?? FONT5[" "]), 0]);
}
